...
namespace PlanetLab.Controls
{
	...
	/// </summary>
	public partial class ControlNodeTagProperties : ControlObjectProperties
	{
		private readonly PlRequest request = new PlRequest(PlRequest.RequestMethod.GetNodeTags);

		/// <summary>
		/// Creates a new control instance.
		/// </summary>
		public ControlNodeTagProperties()
		{
			InitializeComponent();
		}
		...
		{
			// Get the tag.
			PlNodeTag tag = obj as PlNodeTag;

			// Change the display information for the new tag.
			if (null == tag)
			{
				this.ObjectTitle = "Node tag unknown";
				this.Message = "The node tag information is not available.";
			...
			else
			{
				...
				this.textBoxHostname.Text = tag.Hostname;

				// Identifiers.

				this.textBoxTagId.Text = tag.NodeTagId.HasValue ? tag.NodeTagId.Value.ToString() : ControlObjectProperties.notAvailable;
		...
		protected override void OnUpdate(int id)
		{
			...
			this.Message = "Updating the information for node tag {0}...".FormatWith(id);
			this.tabControl.Visible = false;

			// Begin a new tags request for the specified tag.
			this.BeginRequest(this.request, Config.Static.PlanetLabUsername, Config.Static.PlanetLabPassword, PlNodeTag.GetFilter(PlNodeTag.Fields.NodeTagId, id));
		}
		...
		protected override void OnRequestResult(XmlRpcResponse response, RequestState state)
		{
			...
			{
				PlList<PlNodeTag> tags = null;
				// Create a PlanetLab tags list for the given response.
				try { tags = PlList<PlNodeTag>.Create(response.Value as XmlRpcArray); }
				catch { }
		...
		protected override void OnRequestException(Exception exception, RequestState state)
		{
			...
			this.ObjectTitle = "Node tag unknown";
			this.Message = "An error occurred while requesting the node tag information. {0}{1}{2}".FormatWith(
				Environment.NewLine,
				Environment.NewLine,
				exception.Message);
		}
