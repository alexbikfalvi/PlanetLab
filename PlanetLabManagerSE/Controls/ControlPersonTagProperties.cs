...
namespace PlanetLab.Controls
{
	...
	/// </summary>
	public partial class ControlPersonTagProperties : ControlObjectProperties
	{
		private readonly PlRequest request = new PlRequest(PlRequest.RequestMethod.GetPersonTags);

		/// <summary>
		/// Creates a new control instance.
		/// </summary>
		public ControlPersonTagProperties()
		{
			InitializeComponent();
		}
		...
		{
			// Get the tag.
			PlPersonTag tag = obj as PlPersonTag;
			...
			else
			{
				...
				this.textBoxEmail.Text = tag.Email;

				// Identifiers.

				this.textBoxTagId.Text = tag.PersonTagId.HasValue ? tag.PersonTagId.Value.ToString() : ControlObjectProperties.notAvailable;
				this.textBoxPersonId.Text = tag.PersonId.HasValue ? tag.PersonId.Value.ToString() : ControlObjectProperties.notAvailable;
		...
		protected override void OnUpdate(int id)
		{
			...
			this.BeginRequest(this.request, Config.Static.PlanetLabUsername, Config.Static.PlanetLabPassword, PlPersonTag.GetFilter(PlPersonTag.Fields.PersonTagId, id));
		}
		...
		protected override void OnRequestResult(XmlRpcResponse response, RequestState state)
		{
			...
			{
				PlList<PlPersonTag> tags = null;
				// Create a PlanetLab tags list for the given response.
				try { tags = PlList<PlPersonTag>.Create(response.Value as XmlRpcArray); }
				catch { }
